...
public class SkillServiceImpl implements ISkillService {
	...
	public List<Skill> queryOtherSkillsByEmployeeId(Long employeeId) {
		String hql = "select d from Skill d where d.id not in (select cdm.skill.id from EmployeeSkillMapping cdm"
				+ "  where  cdm.employee.id=?0)";
		return skillDao.findByHQL(hql, new Object[] {employeeId});
	}
	...
	public List<Skill> queryOtherSkillsByEmployeeIdAndCondition(Long employeeId, String q) {
		...
		return skillDao.findByHQL(hql, new Object[] {employeeId,"%" + q + "%"});
	}
}
