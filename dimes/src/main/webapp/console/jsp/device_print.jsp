			 ...
			 $.get("device/printQr.do",{ids:ids},function(data){
				...
				while(i<data.length){
					...
					if(data1!=undefined){
						...
						var code = content('设备代码',data1.code);
						var name = content('设备名称',data1.name);
						var unitType = content('规格型号',data1.unitType);
						var installDate = content('安装日期',data1.installDate);
					...
					if(data2!=undefined){
						...
						var code = content('设备代码',data2.code);
						var name = content('设备名称',data2.name);
						var unitType = content('规格型号',data2.unitType);
						var installDate = content('安装日期',data2.installDate);
						$innerBottom.append(code);
						$innerBottom.append(name);
						$innerBottom.append(unitType);
						$innerBottom.append(installDate);
					...
					if(data3!=undefined){
						...
						$innerTop.append("<img src='<%=basePath%>/"
									...
									var code = content('设备代码', data3.code);
									var name = content('设备名称', data3.name);
									var unitType = content('规格型号',
											data3.unitType);
									var installDate = content('安装日期',
											data3.installDate);
									$innerBottom.append(code);
									$innerBottom.append(name);
									$innerBottom.append(unitType);
