...
DROP VIEW ACT_ID_INFO;
 
 /*创建用户组视图*/
CREATE  VIEW  ACT_ID_GROUP AS SELECT R.ROLENAME AS ID_,0 AS REV_,R.ROLENAME AS NAME_,R.TYPE AS TYPE_ FROM ROLE R;
 /*创建用户视图*/
 CREATE  VIEW  ACT_ID_USER AS SELECT U.USERNAME AS ID_,0 AS REV_,U.REALNAME AS FIRST_,U.REALNAME AS LAST_,U.EMAIL AS EMAIL_,U.PASSWORD AS PWD_,U.PICNAME AS PICTRUE_ID FROM T_USER U;
 /*创建用户和组关联视图*/
 CREATE  VIEW  ACT_ID_MEMBERSHIP AS SELECT U.USERNAME AS USER_ID_,R.ROLENAME AS GROUP_ID_ FROM USER_ROLE UR INNER JOIN T_USER U ON UR.USER_ID=U.id
 INNER JOIN ROLE R ON UR.ROLE_ID=R.ID;
 /*创建用户额外信息视图*/
 CREATE  VIEW  ACT_ID_INFO AS SELECT  CONVERT(NVARCHAR(64),UR.ID) AS ID_ ,0 AS REV_ ,U.USERNAME AS USER_ID_,R.TYPE AS TYPE,
NULL AS KEY_ , NULL AS VALUE,U.PASSWORD AS PASSWORD_,NULL AS PARENT_ID FROM USER_ROLE UR INNER JOIN T_USER U ON UR.USER_ID=U.id
 INNER JOIN ROLE R ON UR.ROLE_ID=R.ID;
 
 
 
 /*初始化未分类:生产单元 */
 insert into PRODUCTIONUNIT(id,code,name)values(-1,'未分类','未分类');